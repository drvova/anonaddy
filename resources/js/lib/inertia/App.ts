...
import { formatPageTitle, resolvePageTitle } from '../pageTitle'
...
type InertiaComponent = Component<Page['props']> & {
  ...
  title?: string | ((props: Page['props']) => string)
}
...
export default function App(props: ParentProps<InertiaAppProps>) {
  ...
  if (!isServer) {
    createEffect(() => {
      const title = current.component?.title
      const pageTitle =
        typeof title === 'function'
          ? title(current.page.props)
          : title || resolvePageTitle(current.page.component)
