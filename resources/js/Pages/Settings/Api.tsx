...
export default function ApiSettings(props: { initialTokens: { data: Token[] } }) {
  ...
  const isObject = (val: any) => val && typeof val === 'object' && !Array.isArray(val) && Object.keys(val).length > 0
        ...
        <div class="pt-10">
          <div class="space-y-1">
            ...
            <p class="text-base text-grey-700 dark:text-grey-200">
              ...
              <a href="https://addons.mozilla.org/en-GB/firefox/addon/addy_io/" target="_blank" rel="nofollow noopener noreferrer" class="text-secondary dark:text-indigo-400">Firefox</a>,{' '}
              <a href="https://chrome.google.com/webstore/detail/addyio-anonymous-email-fo/iadbdpnoknmbdeolbapdackdcogdmjpe" target="_blank" rel="nofollow noopener noreferrer" class="text-secondary dark:text-indigo-400">Chrome</a>,{' '}
              <a href="https://microsoftedge.microsoft.com/addons/detail/addyio-anonymous-email/ohjlgpcfncgkijjfmabldlgnccmgcehl" target="_blank" rel="nofollow noopener noreferrer" class="text-secondary dark:text-indigo-400">Edge</a>{' '}
              and{' '}
              <a href="https://apps.apple.com/app/addy-io-extension/id6670220050" target="_blank" rel="nofollow noopener noreferrer" class="text-secondary dark:text-indigo-400">Safari</a>{' '}
              ...
              <a href="/docs" class="text-secondary dark:text-indigo-400">API documentation</a>.
            </p>
          </div>
          <div class="mt-4">
            <button
            ...
            <div class="mt-6">
              ...
              <div>
                <Show when={tokens().length === 0}>
                  <p class="mb-0 text-base text-grey-700 dark:text-grey-200">
                    ...
                    <For each={tokens()}>
                      {(token) => (
                        <div class="table-row even:bg-grey-50 odd:bg-white dark:even:bg-grey-800 dark:odd:bg-grey-900">
                          ...
                          <div class="table-cell p-1 md:p-4">{(window as any).filters?.timeAgo?.(token.created_at) ?? token.created_at}</div>
                          <div class="table-cell p-1 md:p-4">
                            {token.last_used_at ? ((window as any).filters?.timeAgo?.(token.last_used_at) ?? token.last_used_at) : 'Not used yet'}
                          </div>
                          <div class="table-cell p-1 md:p-4">
                            {token.expires_at ? ((window as any).filters?.formatDate?.(token.expires_at) ?? token.expires_at) : 'Does not expire'}
                          </div>
        ...
        <Show when={!accessToken()}>
          <div>
            ...
            <div class="mt-6">
              ...
              <label for="create-token-name" class="block text-sm my-2 font-medium leading-6 text-grey-600 dark:text-white">Name</label>
              <input
                value={formName()}
                ...
                required
              />
              <label for="create-token-expiration" class="block font-medium leading-6 text-grey-600 text-sm my-2 dark:text-white">Expiration</label>
              <div class="block relative">
                ...
                >
                  ...
                  <option class="dark:bg-grey-900" value="">No expiration</option>
                </select>
              </div>
              <Show when={!(page.props as any).usesExternalAuthentication}>
                <label for="create-token-password" class="block text-sm my-2 font-medium leading-6 text-grey-600 dark:text-white">Confirm Password</label>
                <input
                  value={formPassword()}
              ...
              <button
                ...
                class="ml-4 px-4 py-3 text-grey-800 font-semibold bg-white hover:bg-grey-50 dark:text-grey-100 dark:hover:bg-grey-700 dark:bg-grey-600 dark:border-grey-700 border border-grey-100 rounded"
              >
                Close
              </button>
            </div>
        ...
        <Show when={accessToken()}>
          <div>
            ...
            <pre
              ...
              class="w-full bg-grey-100 border border-transparent text-grey-700 rounded p-3 text-md break-all dark:bg-white/5 dark:text-white cursor-pointer"
