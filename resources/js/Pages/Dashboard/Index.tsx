...
interface DashboardProps {
  totals: { total: string | number; active: string | number; inactive: string | number; deleted: string | number; forwarded: string | number; blocked: string | number; replies: string | number; sent: number }
  bandwidthMb: number
...
export default function DashboardIndex(props: DashboardProps) {
  ...
  const stats = [
    { name: 'Shared Domain Aliases', stat: props.aliases, icon: 'at-symbol', url: route('aliases.index', { shared_domain: 'true', active: 'true' }) },
  ...
  const aliasStats = [
    { name: 'Total Aliases', stat: parseInt(String(props.totals.total)), icon: 'at-symbol', url: route('aliases.index', { deleted: 'with' }) },
    { name: 'Active', stat: parseInt(String(props.totals.active)), icon: 'check-circle', url: route('aliases.index', { active: 'true' }) },
    { name: 'Inactive', stat: parseInt(String(props.totals.inactive)), icon: 'cross-circle', url: route('aliases.index', { active: 'false' }) },
    { name: 'Deleted', stat: parseInt(String(props.totals.deleted)), icon: 'trash', url: route('aliases.index', { deleted: 'only' }) },
  ]
  ...
  return (
    <div>
      ...
      {Number(bandwidthPercentage()) === 100 && (
        <div class="text-base border-t-8 rounded text-yellow-800 border-yellow-600 bg-yellow-100 px-3 py-4 mb-4" role="alert">
          <div class="flex items-center mb-2">
            <span class="rounded-full bg-yellow-400 uppercase px-2 py-1 text-xs font-bold mr-2">Warning</span>
            <div>
      ...
      <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-5">
        {stats.map((item) => (
          <div class="relative overflow-hidden rounded-lg bg-white px-4 pb-12 pt-5 shadow sm:px-6 sm:pt-6 dark:bg-grey-900">
            <dt>
              <div class="absolute rounded-md bg-primary p-3">
                <Icon name={item.icon} class="h-6 w-6 text-white" />
              </div>
            ...
            <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
              <p class="text-2xl font-semibold text-grey-900 dark:text-grey-100">
      ...
      <div class="mt-6">
        <div class="overflow-hidden rounded-full bg-grey-100 relative dark:bg-grey-900">
          ...
          <span class="h-8 absolute top-0 right-0 flex items-center pr-4 text-grey-600 font-semibold dark:text-grey-200">
            ...
            <div class="relative overflow-hidden rounded-lg bg-white px-4 pb-12 pt-5 shadow sm:px-6 sm:pt-6 dark:bg-grey-900">
              ...
              <dd class="ml-16 flex items-baseline pb-6 sm:pb-7">
                ...
                <div class="absolute inset-x-0 bottom-0 bg-grey-50 px-4 py-4 sm:px-6 dark:bg-grey-900">
                  <div class="text-sm">
                    <Link href={item.url} class="font-medium text-primary hover:text-primary/80">
                      {idx === 0 ? 'View All' : `View ${item.name}`}<span class="sr-only"> {item.name} stats</span>
                    </Link>
                  </div>
                </div>
              </dd>
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {emailStats.map((item) => (
            <div class="relative overflow-hidden rounded-lg bg-white p-4 shadow dark:bg-grey-900">
              <dt>
                <Icon name={item.icon} class="inline-block w-16 h-16 text-primary/10 absolute top-0 right-0" />
                <p class="truncate text-sm font-medium text-grey-400 dark:text-grey-200">
                  {item.name}
                </p>
              </dt>
              <dd class="flex items-baseline">
                <p class="text-3xl font-semibold text-grey-900 dark:text-grey-100">
                  {item.stat.toLocaleString()}
                </p>
              </dd>
            </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div class="mt-5 bg-white overflow-hidden shadow sm:rounded-lg p-4 flex justify-center max-h-80 dark:bg-grey-900">
        ...
        <div class="mt-5 bg-white overflow-hidden shadow sm:rounded-lg p-4 flex justify-center max-h-80 dark:bg-grey-900">
          <Show when={outboundMessageTotals()} fallback={<div class="flex items-center justify-center">No data to display</div>}>
